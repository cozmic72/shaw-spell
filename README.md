...
**A missing `shave` is caught up front.** Four of the seven call sites — in `generate_britfone_supplement.py`, `generate_wordnet_supplement.py`, `generate_wiktionary_supplement.py` and `rescore_supplements.py` — catch `FileNotFoundError` alongside a timeout and return an empty result. That is the right response to a timeout, but on its own it was the wrong one to an absent binary: every batch took that branch, the generator warned on stderr but exited 0, and it wrote a pool with every `shave` consultation dropped, in a file nothing distinguished from a good one.

Those four entry points now call `require_shave()` (`src/tools/shave_tool.py`), which exits non-zero if the binary is not on `$PATH`, and the build targets that can reach a call site depend on the `require-shave` make target. The absence is therefore detected once, before any work begins, whether you go through `make` or run a generator directly. A *timeout* is still absorbed per-batch and still exits 0 — one oversized batch giving up need not abort a multi-minute run — so a run over a pathologically slow `shave` can still under-consult it.
